...
int main(int argc, char *argv[]) {
    ...
    while (true) {
        ...
        if (clientSocket == -1) {
            ...
            continue; // Skip and keep accepting new connections
        }
        ...
        if (pid == 0) { // Child process
            close(serverSocket); // Close the parent's listening socket in the child
            ...
            close(clientSocket);
            _exit(0); // Terminate the child process after handling the client
        } else { // Parent process
    ...
    // Close the server socket (in parent process only)
    close(serverSocket);
