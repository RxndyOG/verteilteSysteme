...
int calcDirectrion(INFOStruct *is){
    switch(is->argument){
        case SEND:
            break;
        case READ:
...
void parseINFOstring(INFOStruct *is){
    ...
    for(int i = 0; i < argArray.size(); i++){   //infoString formatierung -> argument\nusername\ntextLength\npackageNUM\n
        if(i == 0){is->argument = std::stoi(argArray[i]);}
        if(i == 1){is->username = argArray[i];}
        if(i == 2){is->textLength = std::stoi(argArray[i]);}
        if(i == 3){is->packageNUM = std::stoi(argArray[i]);}
    }
}
...
int recvAndParseInfoString(INFOStruct *is, int clientSocket){
    INFOStruct is;
    int *returnValue;
    is->INFOstring = recvFunctBasic(clientSocket, returnValue);
    if(*returnValue != -1){
        sendFunctBasic("OK\n", clientSocket);   // returned OK
        parseINFOstring(is);
        return is->argument;
    }
