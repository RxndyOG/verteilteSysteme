...
void handleClient(int clientSocket, sockaddr_in clientAddress, std::vector<struct TextPreset> &savedMSG, const std::string &filePath)
{
    ...
    std::string userPWD = basicFunktions().recvFunctBasic(clientSocket);
    std::array<std::string, 2> userPWDarr = {};
    int i = 0;
    for(auto &c : userPWD){
        if(c == '\n'){
            i++;
        }else{
            userPWDarr[i].push_back(c);
        }
    }

    std::string username = userPWDarr[0];
    std::string pwd = userPWDarr[1];
    ...
    {
        std::string arg = FileHandeling().readFileToString(filePath, username);
    ...
    do
    {
        cancell = recvFromClient(clientSocket, savedMSG, filePath);
    } while (cancell != QUIT);
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        std::thread clientThread(handleClient, clientSocket, clientAddress, std::ref(savedMSG), argv[2]);

        // Thread im Hintergrund weiterlaufen lassen
        clientThread.detach();
    }
